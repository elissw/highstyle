# Load the package
#------------------
# Make sure to check where you're running from and that the path to highstyle is not broken
devtools::load_all("../highstyle")
...
plot_actind <- ggplot2::ggplot(df,ggplot2::aes(y=activation_index))+
  geom_bar_highstyle()+ theme_barplot_axes
print(plot_actind)
# geom_bar_highstyle is a wrapper function for geom_bar, it makes sure the bars are 
...
plot <- ggplot2::ggplot(df_counts)+
  # geom_lollipop_highstyle is a custom ggplot geometry combining
  # geom_segment and geom_point in order to achieve the lollipop look
  geom_lollipop_highstyle(ggplot2::aes(x=0,xend=n,
